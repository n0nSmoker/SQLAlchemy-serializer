...
def test_rules_param_got_in_runtime(get_instance):
    i = get_instance(NestedModel, model_id=get_instance(FlatModel).id)
    data = i.to_dict(
        rules=("-id", "prop", "-model.id", "model._protected_method", "model.prop")
    )
...
def test_overlapping_of_default_and_got_in_runtime_params1(get_instance):
    ...
    data = i.to_dict(
        only=(
            ...
            "model.method",
            "model.prop",
        )  # Params passed in to_dict have HIGHEST priority
    )
...
def test_combination(get_instance):
    ...
    res = nested.to_dict(
        only=("model", "set"), rules=("model.set", "-model.id", "-model.date")
    )
...
def test_combination2(get_instance):
    ...
    assert set(res.keys()) == {"model", "set"}
    assert set(res["model"].keys()) == {"string", "time", "set"}
